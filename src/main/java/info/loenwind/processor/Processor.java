...
public class Processor extends AbstractProcessor {
    ...
    // Forge
    private final static ClassName EVENT_BUS_SUBSCRIBER = ClassName.get("net.minecraftforge.fml.common.Mod",
            "EventBusSubscriber");
    private final static ClassName SUBSCRIBE_EVENT = ClassName.get("net.minecraftforge.fml.common.eventhandler",
            "SubscribeEvent");
    private final static ClassName I_FORGE_REGISTRY_ENTRY__IMPL = ClassName.get("net.minecraftforge.registries",
            "IForgeRegistryEntry", "Impl");
    private final static ClassName REGISTRY_EVENT__REGISTER = ClassName.get("net.minecraftforge.event.RegistryEvent",
            "Register");
    private final static ClassName BYTE_BUF_UTILS = ClassName.get("net.minecraftforge.fml.common.network",
            "ByteBufUtils");
    private final static ClassName MOD = ClassName.get("net.minecraftforge.fml.common", "Mod");

    // Ender IO
    private final static ClassName I_REMOTE_EXEC = ClassName.get("crazypants.enderio.base.network.ExecPacket",
            "IServerExec");
                    ...
                    if (typeUtils.isAssignable(typeMirror, containerType)) {
                        try {
                            ...
                            processingEnv.getMessager().printMessage(Diagnostic.Kind.OTHER,
                                    "Internal error: " + e.getMessage(), element);
                            e.printStackTrace();
                        }
                    } else {
                        processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                "Unsupported type '" + typeMirror + "' for @RemoteCall", element);
                    }
                }
            }
        }
        ...
        while (true) {
            ...
            if (e instanceof PackageElement) {
                ...
                if (!qualifiedName.toString().contains(".")) {
                    // technically there are still 2 packages to look at, but we never go that far up with our mod
                    // classes
                    return null;
                }
            ...
            for (Element c : e.getEnclosedElements()) {
                for (AnnotationMirror m : c.getAnnotationMirrors()) {
                    if (MOD.equals(ClassName.get(m.getAnnotationType()))) {
                        for (Entry<? extends ExecutableElement, ? extends AnnotationValue> b : m.getElementValues()
                                .entrySet()) {
    ...
    private void generateContainerProxy(TypeElement typeElement) throws IOException {
        ...
        if (modid == null || modid.isEmpty()) {
            ...
            if (modid == null || modid.isEmpty()) {
                processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                        "Parameter 'modid' must be set on class level if there's no @Mod class in hierarchy",
                        typeElement);
                return;
            }
        }
        ...
        for (ExecutableElement method : processingEnv.getElementUtils().getAllMembers(typeElement).stream()
                .filter(el -> el.getKind() == ElementKind.METHOD)
                .map(el -> (ExecutableElement) el).filter(el -> el.getAnnotation(RemoteCall.class) != null)
                .collect(Collectors.toList())) {
            String methodName = method.getSimpleName().toString();
            String methodProxyClassName = proxyInterfaceClassName + "$" + methodName;

            if (method.getAnnotation(RemoteCall.class).modid() != null &&
                    !method.getAnnotation(RemoteCall.class).modid().isEmpty()) {
                processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                        "Parameter 'modid' must NOT be set on method level", method);
                return;
            }

            FieldSpec instanceField = FieldSpec
                    .builder(I_REMOTE_EXEC, "INSTANCE", Modifier.STATIC, Modifier.FINAL, Modifier.PUBLIC)
                    .initializer("new $1L().setRegistryName($2S, $3S)", methodProxyClassName, modid,
                            (sourceClassName + "_" + methodName).toLowerCase(Locale.ENGLISH))
                    .build();

            TypeSpec.Builder methodProxyClassBuilder = TypeSpec.classBuilder(methodProxyClassName)
                    .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                    .addAnnotation(AnnotationSpec.builder(EVENT_BUS_SUBSCRIBER).addMember("modid", "$S", modid).build())
                    .superclass(ParameterizedTypeName.get(I_FORGE_REGISTRY_ENTRY__IMPL, I_REMOTE_EXEC))
                    .addSuperinterface(I_REMOTE_EXEC).addField(instanceField)
            ...
            MethodSpec.Builder proxyMethodBuilder = MethodSpec.methodBuilder(methodName)
                    .addModifiers(Modifier.PUBLIC, Modifier.DEFAULT).returns(void.class);

            CodeBlock.Builder parameterListCode = null;

            MethodSpec.Builder writerBuilder = MethodSpec.methodBuilder("makeWriter")
                    .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
            ...
            MethodSpec.Builder runnerBuilder = MethodSpec.methodBuilder("apply").addModifiers(Modifier.PUBLIC)
                    .addAnnotation(Override.class)
                    .returns(ParameterizedTypeName.get(CONSUMER, ENTITY_PLAYER_MP)).addParameter(BYTE_BUF, "buf")
                    .addStatement("final int _windowId = buf.readInt()");
                ...
                if (typeName.isPrimitive()) {
                    ...
                    if (!isNonnull(parameter)) {
                        try {
                            ...
                            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                    "Too many nullable parameters", parameter);
                        }
                    }
                    if (typeName.isBoxedPrimitive()) {
                        ...
                        b.addReaderStatement(typeName, parameterSpec, "buf.read$1L()", byteBuffCall);
                            ...
                            ((DeclaredType) paramTypeMirror).asElement().getKind() == ElementKind.ENUM) {
                                b.addWriterStatement("buf.writeInt($1T.put($2N))", ENUM_READER, parameterSpec);
                                b.addReaderStatement(typeName, parameterSpec,
                                        "$1T.get($2T.class, buf.readInt())", ENUM_READER, typeName);
                            } else
                        if (typeName.equals(STRING)) {
                            ...
                            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                    "Cannot serialize '" + typeName + "' into a byte stream", parameter);
                            return;
                        }
                    b.endNullable();
                }
            ...
            runnerBuilder.beginControlFlow("return player ->")
                    .beginControlFlow(
                            "if (player.openContainer instanceof $T && player.openContainer.windowId == _windowId)",
            ...
            proxyMethodBuilder.addCode("$1T.send($3N.$2N, $3N.$4N(", EXEC_PACKET, instanceField, methodProxyClass,
                    makeWriterMethod);
            if (parameterListCode != null) {
    ...
    private static ParameterSpec ParameterSpec_get(VariableElement element) {
        return ParameterSpec.builder(TypeName.get(element.asType()), element.getSimpleName().toString())
                .addModifiers(element.getModifiers())
                .addAnnotations(element.getAnnotationMirrors().stream().map(AnnotationSpec::get)
                        .collect(Collectors.toList()))
                .build();
    }

    /**
     * {@link #findModid(Element)} would find these on its own in a full compile but Eclipse's partial compile of single
     * changed class files hides the mod class
     * from us. So we keep this precompiled list here to avoid having to do a complete project build every single time
     * we change one of the remote call classes...
     */
