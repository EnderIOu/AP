...
public class LangProcessor extends AbstractProcessor {
    ...
    private void generateLangFile(TypeElement element) throws IOException, URISyntaxException {
        ...
        // we cannot call annotation.mod() as that would want to give us a Class object, which is not accessible at this
        // point. So we need to dig a bit to get that
        // parameter value as a TypeMirror instead...
        TypeMirror modClass = findAnnotationClassParameter(element);
        if (modClass == null) {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                    "Couldn't find class given in parameter 'mod'", element);
            return;
        }
        ...
        if (resource == null) {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "Couldn't find source folder root",
                    element);
            return;
        } else if (!resource.isFile()) {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "Resource '" + file + "' is not a file",
                    element);
            return;
        } else if (!resource.exists()) {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "Resource '" + file + "' does not exist",
                    element);
            return;
        } else if (!resource.canRead()) {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "Resource '" + file + "' is not readable",
                    element);
            return;
        }
        ...
        try (InputStream input = new FileInputStream(resource)) {
            ...
            prop.forEach((key, langstring) -> {
                ...
                if (!value.startsWith("tile.") && !value.startsWith("item.") && !value.startsWith("entity.")) {
                    ...
                    String name = value.replaceFirst("^" + modid + "\\.", ".").replaceAll("[^a-zA-Z0-9]", "_")
                            .replaceAll("__+", "_").replaceFirst("^_", "")
                            .toUpperCase(Locale.ENGLISH);
                    enumBuilder.addEnumConstant(name,
                            TypeSpec.anonymousClassBuilder("$S, $L, $L", value, placeholders, isMultiLine).build());
                }
            });
        }

        final TypeName nnString = ClassName.get(String.class)
                .annotated(AnnotationSpec.builder(ClassName.get("javax.annotation", "Nonnull")).build());
        ...
        enumBuilder.addMethod(MethodSpec.methodBuilder("getLang").addModifiers(Modifier.PUBLIC) //
                .addAnnotation(ClassName.get(Override.class)) //
                .returns(ClassName.get("com.enderio.core.common", "Lang")
                        .annotated(AnnotationSpec.builder(ClassName.get("javax.annotation", "Nonnull")).build())) //
    ...
    private TypeMirror findAnnotationClassParameter(TypeElement element) {
        final TypeMirror LANG_BUILDER = processingEnv.getElementUtils()
                .getTypeElement(LangBuilder.class.getCanonicalName()).asType();
                ...
                for (Entry<? extends ExecutableElement, ? extends AnnotationValue> b : m.getElementValues()
                    ...
                    if (b.getKey().getSimpleName().toString().equals("mod")) {
                        ...
                        if (x instanceof TypeMirror) {
                            ...
                            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                                    "How did you assign something that's not a Class to a field of type Class<?>? ",
                                    element);
                        }
                    }
                }
    ...
    /**
     * Returns the String value of the parameter 'modid' of the annotation 'net.minecraftforge.fml.common.Mod' that is
